...
contract Marketplace {
    ...
    function addAccount(address _accountAddr) public {
        ...
        require(tempAccount.getMarketplace() == this, "The account belongs to a different marketplace");
        require(owners[tempAccount.owner()] == address(0), "This address already owns an account on this marketplace");

        uint accountType = tempAccount.getAccountType();

        if(accountType == 1) { // funder, for manager there is nothing to be done
            // send the inital tokens to the funder
            token.approve(address(this), token.getTokensPerFunder());
            token.transferFrom(address(this), _accountAddr, token.getTokensPerFunder());
        } else if(accountType == 2) { // reviewer
    ...
    function initializeTask(string memory description, string memory domain, uint totalFreelancerBounty, uint totalReviewerBounty) public {
        require(owners[Account(msg.sender).owner()] != address(0), "This manager hasn't been added to the marketplace.");
        require(Account(msg.sender).getAccountType() == 0, "This address is not associated with a manager account.");
    ...
    function cancelTask(Task task) public {
        ...
        uint taskIndex = getTaskIndex(task);
        require(taskIndex < taskList.length, "This task cannot be found in the task list"); // works also as a check for freelancer to be part of this marketplace
    ...
    function provideFunding(Task task, uint nTokens) public {
        require(owners[Account(msg.sender).owner()] != address(0), "This funder hasn't been added to the marketplace.");
        require(Account(msg.sender).getAccountType() == 1, "This address is not associated with a funder account.");
